// Optional vector-embedding layer for semantic retrieval (mirrors the experience-ledger
// leaf; each MCP server is self-contained, so the small embedder is copied, not shared).
//
// Zero hard dependency: API providers use the built-in global `fetch`, plus a deterministic
// offline local embedder. Opt-in via DOCS_RAG_EMBED; unset = purely lexical, no network.
...
/**
 * Deterministic, dependency-free, OFFLINE embedder via feature hashing of tokens (signed
 * hashing into a fixed-width L2-normalized vector). Stable and network-free, so it powers
 * offline tests and a "DOCS_RAG_EMBED=hash" mode. NOT a trained model: similarity is close
 * to lexical overlap; for real semantic recall use an API provider (voyage/openai).
 */
...
async function httpEmbed(url: string, apiKey: string, model: string, texts: string[]): Promise<number[][]> {
...
export function voyageEmbedder(opts: { apiKey: string; model?: string }): Embedder {
  ...
  return { id: `voyage:${model}`, embed: (texts) => httpEmbed("https://api.voyageai.com/v1/embeddings", opts.apiKey, model, texts) };
}

/** OpenAI embeddings. Needs OPENAI_API_KEY. */
export function openAIEmbedder(opts: { apiKey: string; model?: string }): Embedder {
  const model = opts.model ?? "text-embedding-3-small";
  return { id: `openai:${model}`, embed: (texts) => httpEmbed("https://api.openai.com/v1/embeddings", opts.apiKey, model, texts) };
}

/**
 * Build an embedder from the environment, or return null to stay lexical-only:
 *   DOCS_RAG_EMBED = voyage | openai | hash | none (default: none)
 *   VOYAGE_API_KEY / OPENAI_API_KEY  — provider key
 *   DOCS_RAG_EMBED_MODEL             — optional model override
...
export function embedderFromEnv(): Embedder | null {
  const kind = (process.env.DOCS_RAG_EMBED ?? "none").toLowerCase();
  const model = process.env.DOCS_RAG_EMBED_MODEL || undefined;
  if (kind === "none" || kind === "") return null;
  if (kind === "hash") return hashEmbedder();
  if (kind === "voyage") {
    const apiKey = process.env.VOYAGE_API_KEY;
    if (!apiKey) {
      console.error("DOCS_RAG_EMBED=voyage but VOYAGE_API_KEY is unset — falling back to lexical retrieval.");
      return null;
    }
  ...
  if (kind === "openai") {
    ...
    if (!apiKey) {
      console.error("DOCS_RAG_EMBED=openai but OPENAI_API_KEY is unset — falling back to lexical retrieval.");
      return null;
    }
    return openAIEmbedder({ apiKey, model });
  }
  console.error(`Unknown DOCS_RAG_EMBED="${kind}" — falling back to lexical retrieval.`);
  return null;
}
