// Network layer: X (thread, optional community) and LinkedIn (single post) publishers.
// The HTTP transport is injectable (and returns response headers — LinkedIn returns the
// new post id in the `x-restli-id` header) so all posting logic is unit-tested offline.
