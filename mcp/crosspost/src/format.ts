// Pure, dependency-free formatting: Slack mrkdwn cleanup + per-platform packaging.
// No network, no state — heavily unit-tested. (X weighting mirrors the x-poster leaf;
// each MCP leaf is self-contained, so the small helpers are copied, not imported.)
...
// ─── X weighted length + thread split (mirrors x-poster) ─────────────────────────
